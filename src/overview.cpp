...
static bool hookVisibleOnMonitor(void *thisptr, PHLMONITOR monitor) {
  ...
  return ((origVisibleOnMonitor)(g_pVisibleOnMonitorHook->m_original))(thisptr,
                                                                       monitor);
}
...
static void hookRenderLayer(void *thisptr, PHLLS layer, PHLMONITOR monitor,
  ...
  if (overview_enabled) {
    ...
    g_pHyprRenderer->m_renderPass.add(makeShared<OverviewPassElement>(
        OverviewPassElement::OverviewModifData(modif_data)));
    g_pHyprRenderer->damageMonitor(monitor);
  }
  ((origRenderLayer)(g_pRenderLayerHook->m_original))(thisptr, layer, monitor,
                                                      time, popups);
  if (overview_enabled) {
    g_pHyprRenderer->m_renderPass.add(makeShared<OverviewPassElement>(
...
static Vector2D hookGetCursorPosForMonitor(void *thisptr, PHLMONITOR monitor) {
  ...
  Vector2D pos =
      ((origGetCursorPosForMonitor)(g_pGetCursorPosForMonitorHook->m_original))(
          thisptr, monitor);
  if (overview_enabled) {
...
static void hookRenderSoftwareCursorsFor(void *thisptr, PHLMONITOR monitor,
  ...
  if (monitor == last) {
    ...
    if (overviews->overview_enabled(workspace)) {
      ...
      ((origRenderSoftwareCursorsFor)(g_pRenderSoftwareCursorsForHook
                                          ->m_original))(thisptr, monitor, now,
                                                         damage, overridePos);
      monitor->m_size = monitor_size;
    }
  ...
  ((origRenderSoftwareCursorsFor)(g_pRenderSoftwareCursorsForHook->m_original))(
      thisptr, monitor, now, damage, overridePos);
}
...
static PHLMONITOR hookGetMonitorFromVector(void *thisptr,
  ...
  for (auto const &m : compositor->m_monitors) {
    float dist = vecToRectDistanceSquared(point, m->m_position,
                                          m->m_position + m->m_size);
