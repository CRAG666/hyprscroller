...
Column::Column(const Row *pRow, const Column *column,
               List<Window *> &pWindows) {
  width = column->width;
...
void Column::recalculate_col_geometry(const Vector2D &gap_x, double gap,
  ...
  if (row->get_mode_modifier().get_center_window().value() &&
      row->get_active_column() == this) {
  ...
  if (a_y0 < max.y) {
    // active starts above, set it on the top edge, unless it is the last one
    // and there are more, then move it to the bottom
    ...
    // active overflows below the bottom, move to bottom of viewport, unless it
    // is the first window and there are more, then move it to the top
    ...
    if (reorder == Reorder::Auto) {
      ...
      if (!keep_current) {
        ...
        if (active->next() != nullptr) {
          if (std::round(active->data()->get_geom_h() +
                         active->next()->data()->get_geom_h()) <= max.h) {
            // set next at the bottom edge of the viewport
            active->data()->move_to_pos(
                geom.x,
                max.y + max.h - active->data()->get_geom_h() -
                    active->next()->data()->get_geom_h(),
                gap_x, gap0);
          } else if (active->prev() != nullptr) {
            if (std::round(active->prev()->data()->get_geom_h() +
                           active->data()->get_geom_h()) <= max.h) {
              // set previous at the top edge of the viewport
              active->data()->move_to_pos(
                  geom.x, max.y + active->prev()->data()->get_geom_h(), gap_x,
                  gap0);
            } else {
          ...
          if (std::round(active->prev()->data()->get_geom_h() +
            ...
            // it doesn't fit and nothing above, move active to bottom of
            // viewport
            active->data()->move_to_bottom(geom.x, max, gap_x, gap0);
          }
        } else {
...
void Column::recalculate_col_geometry_overview(const Vector2D &gap_x,
                                               double gap) {
...
Window *Column::expel_active(const Vector2D &gap_x) {
  ...
  if (windows.size() == 1) {
    double maxw = width == StandardSize::Free
                      ? active->data()->get_geom_w(gap_x)
                      : row->get_max().w;
...
void Column::align_window(Direction direction, const Vector2D &gap_x,
                          double gap) {
  const Box &max = row->get_max();
  auto gap0 = active == windows.first() ? 0.0 : gap;
...
void Column::cycle_size_active_window(int step, const Vector2D &gap_x,
                                      double gap) {
  reorder = Reorder::Auto;
  ...
  if (height == StandardSize::Free) {
    ...
    if (**CYCLESIZE_CLOSEST) {
      ...
      height = scroller_sizes.get_window_closest_height(
          g_pCompositor->m_lastMonitor, fraction, step);
    } else {
      height = scroller_sizes.get_window_default_height(
          active->data()->get_window());
    }
  } else {
...
void Column::size_active_window(StandardSize height, const Vector2D &gap_x,
                                double gap) {
  reorder = Reorder::Auto;
  active->data()->update_height(height, row->get_max().h);
  recalculate_col_geometry(gap_x, gap, true);
...
void Column::resize_active_window(const Vector2D &gap_x, double gap,
  ...
  if (std::abs(static_cast<int>(delta.y)) > 0) {
    for (auto win = windows.first(); win != nullptr; win = win->next()) {
      ...
      if (!win->data()->can_resize_height(max.h, win == active, gap0, gap1,
                                          delta.y))
        return;
    }
  }
...
void Column::adjust_windows(ListNode<Window *> *win, const Vector2D &gap_x,
                            double gap, bool animate) {
  // 2. adjust positions of windows above
  for (auto w = win->prev(), p = win; w != nullptr; p = w, w = w->prev()) {
    auto gap0 = w == windows.first() ? 0.0 : gap;
    w->data()->move_to_pos(geom.x,
                           p->data()->get_geom_y(gap) - w->data()->get_geom_h(),
                           gap_x, gap0);
  }
  ...
  for (auto w = win->next(), p = win; w != nullptr; p = w, w = w->next()) {
    ...
    w->data()->move_to_pos(
        geom.x, p->data()->get_geom_y(gap0) + p->data()->get_geom_h(), gap_x,
        gap);
  }
  for (auto w = windows.first(); w != nullptr; w = w->next()) {
    auto gap0 = w == windows.first() ? 0.0 : gap;
...
void Column::selection_toggle() { active->data()->selection_toggle(); }
...
Column *Column::selection_get(const Row *row) {
  ...
  PHLWORKSPACE workspace =
      g_pCompositor->getWorkspaceByID(row->get_workspace());
  while (win != nullptr) {
    auto next = win->next();
    if (win->data()->is_selected()) {
