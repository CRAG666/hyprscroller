...
StandardSize ScrollerSizes::get_next(const std::vector<StandardSize> &sizes,
  ...
  static auto *const *CYCLESIZE_WRAP =
      (Hyprlang::INT *const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:cyclesize_wrap")
          ->getDataStaticPtr();
  if (**CYCLESIZE_WRAP)
...
StandardSize ScrollerSizes::get_next_window_height(StandardSize size,
                                                   int step) {
  update();
...
StandardSize ScrollerSizes::get_size(const std::vector<StandardSize> &sizes,
                                     int index) const {
  int current =
      std::min(std::max(0, index), static_cast<int>(sizes.size()) - 1);
  return sizes[current];
}
...
StandardSize ScrollerSizes::get_window_closest_height(PHLMONITORREF monitor,
                                                      double fraction,
                                                      int step) {
  update();
  for (const auto monitor_data : monitors) {
...
StandardSize ScrollerSizes::get_column_closest_width(PHLMONITORREF monitor,
                                                     double fraction,
                                                     int step) {
  update();
  for (const auto monitor_data : monitors) {
...
void ScrollerSizes::update_sizes(std::vector<StandardSize> &sizes,
...
void ScrollerSizes::update() {
  static auto const *window_default_height_str =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:window_default_height")
          ->getDataStaticPtr();
  static auto const *column_default_width_str =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:column_default_width")
          ->getDataStaticPtr();
  static auto const *window_heights_str =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:window_heights")
          ->getDataStaticPtr();
  static auto const *column_widths_str =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:column_widths")
          ->getDataStaticPtr();
  static auto const *monitor_modes_str =
      (Hyprlang::STRING const *)HyprlandAPI::getConfigValue(
          PHANDLE, "plugin:scroller:monitor_options")
          ->getDataStaticPtr();
  ...
  window_default_height =
      get_size_from_string(*window_default_height_str, StandardSize::One);
  column_default_width =
      get_size_from_string(*column_default_width_str, StandardSize::OneHalf);
                        ...
                        if (!option_data.empty()) {
                          if (option_name == "mode") {
                            if (option_data == "r" || option_data == "row") {
                              monitor_data.mode = Mode::Row;
                            } else if (option_data == "c" ||
                                       option_data == "col" ||
                                       option_data == "column") {
                              monitor_data.mode = Mode::Column;
                            }
                          } else if (option_name == "column_default_width") {
                            monitor_data.column_default_width =
                                get_size_from_string(option_data,
                                                     StandardSize::OneHalf);
                          } else if (option_name == "window_default_height") {
                            monitor_data.window_default_height =
                                get_size_from_string(option_data,
                                                     StandardSize::One);
                          } else if (option_name == "column_widths") {
                            update_sizes(monitor_data.column_widths,
                                         option_data, StandardSize::OneHalf);
                          } else if (option_name == "window_heights") {
                            update_sizes(monitor_data.window_heights,
                                         option_data, StandardSize::One);
                          }
                        }
                      }
                    }
...
StandardSize ScrollerSizes::get_size_from_string(const std::string &size,
                                                 StandardSize default_size) {
  if (size == "one") {
