...
SDispatchResult dispatch_cyclewidth(std::string arg) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:cyclewidth: invalid workspace"};
...
SDispatchResult dispatch_cycleheight(std::string arg) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:cycleheight: invalid workspace"};
...
SDispatchResult dispatch_movewindow(std::string arg) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:movewindow: invalid workspace"};
...
SDispatchResult dispatch_alignwindow(std::string arg) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:alignwindow: invalid workspace"};
...
SDispatchResult dispatch_admitwindow(std::string arg) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:admitwindow: invalid workspace"};
...
SDispatchResult dispatch_expelwindow(std::string arg) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:expelwindow: invalid workspace"};
...
SDispatchResult dispatch_setmodemodifier(std::string arg) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:setmodemodifier: invalid workspace"};
...
SDispatchResult dispatch_toggleoverview(std::string) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:toggleoverview: invalid workspace"};
...
SDispatchResult dispatch_marksdelete(std::string arg) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error =
                "scroller:marksdelete: called while not running hyprscroller"};
...
SDispatchResult dispatch_marksvisit(std::string arg) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error =
                "scroller:marksvisit: called while not running hyprscroller"};
...
SDispatchResult dispatch_marksreset(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error =
                "scroller:marksreset: called while not running hyprscroller"};
...
SDispatchResult dispatch_selectiontoggle(std::string) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:selectiontoggle: invalid workspace"};
...
SDispatchResult dispatch_selectionreset(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error =
            "scroller:selectionreset: called while not running hyprscroller"};
...
SDispatchResult dispatch_selectionworkspace(std::string) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:selectionworkspace: invalid workspace"};
...
SDispatchResult dispatch_selectionmove(std::string arg) {
  ...
  if (workspace == -1)
    return {.success = false,
            .error = "scroller:selectionmove: invalid workspace"};
...
SDispatchResult dispatch_trailnew(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error =
                "scroller:trailnew: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailnext(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error =
                "scroller:trailnext: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailprev(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error =
            "scroller:trailprevious: called while not running hyprscroller"};
...
SDispatchResult dispatch_traildelete(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error =
                "scroller:traildelete: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailclear(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error =
                "scroller:trailclear: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailtoselection(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error =
            "scroller:trailtoselection: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailmarktoggle(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error =
            "scroller:trailmarktoggle: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailmarknext(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {
        .success = false,
        .error =
            "scroller:trailmarknext: called while not running hyprscroller"};
...
SDispatchResult dispatch_trailmarkprev(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error = "scroller:trailmarkprevious: called while not running "
                     "hyprscroller"};
...
SDispatchResult dispatch_jump(std::string) {
  if (g_pLayoutManager->getCurrentLayout() != g_ScrollerLayout.get())
    return {.success = false,
            .error = "scroller:jump: called while not running hyprscroller"};
...
void addDispatchers() {
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:cyclesize",
                               dispatch_cyclesize);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:cyclewidth",
                               dispatch_cyclewidth);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:cycleheight",
                               dispatch_cycleheight);
  ...
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:setheight",
                               dispatch_setheight);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:movefocus",
                               dispatch_movefocus);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:movewindow",
                               dispatch_movewindow);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:alignwindow",
                               dispatch_alignwindow);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:admitwindow",
                               dispatch_admitwindow);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:expelwindow",
                               dispatch_expelwindow);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:setmode", dispatch_setmode);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:setmodemodifier",
                               dispatch_setmodemodifier);
  ...
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:fitheight",
                               dispatch_fitheight);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:toggleoverview",
                               dispatch_toggleoverview);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:marksadd", dispatch_marksadd);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:marksdelete",
                               dispatch_marksdelete);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:marksvisit",
                               dispatch_marksvisit);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:marksreset",
                               dispatch_marksreset);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:pin", dispatch_pin);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:selectiontoggle",
                               dispatch_selectiontoggle);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:selectionreset",
                               dispatch_selectionreset);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:selectionworkspace",
                               dispatch_selectionworkspace);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:selectionmove",
                               dispatch_selectionmove);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:trailnew", dispatch_trailnew);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:trailnext",
                               dispatch_trailnext);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:trailprevious",
                               dispatch_trailprev);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:traildelete",
                               dispatch_traildelete);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:trailclear",
                               dispatch_trailclear);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:trailtoselection",
                               dispatch_trailtoselection);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:trailmarktoggle",
                               dispatch_trailmarktoggle);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:trailmarknext",
                               dispatch_trailmarknext);
  HyprlandAPI::addDispatcherV2(PHANDLE, "scroller:trailmarkprevious",
                               dispatch_trailmarkprev);
